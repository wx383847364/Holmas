# 热更新边界规范 v1

## Summary

这份 skill 的目标不是解释 Unity 常识，而是给这个项目提供一套**强边界约束**，让后续主 agent 和 subagent 在开发时始终遵循：

- `App.AOT` 不写业务
- `App.Shared` 只放稳定接口和 DTO
- `App.HotUpdate` 承载正式玩法
- `MinesweeperTerrainData` 是地图模板，不是运行时状态
- YooAssets 是正式运行时资源入口
- UI 只做表现和交互，不写核心生成与结算

建议将 skill 放在：

- `$CODEX_HOME/skills/unity-hotupdate-boundary/`

并采用最小结构：
- `SKILL.md`
- `references/boundaries.md`
- `references/checklist.md`
- `tools/validation/check_boundary.sh`
- `agents/openai.yaml`

## Draft

### `SKILL.md` 草案

```md
---
name: unity-hotupdate-boundary
description: Use for this Unity + HybridCLR + YooAssets project when implementing or reviewing gameplay code, module boundaries, HotUpdate architecture, cross-layer DTOs, resource loading, or subagent task splits. Enforces App.AOT/App.Shared/App.HotUpdate responsibilities, keeps gameplay logic out of UI and AOT, and treats MinesweeperTerrainData as a board template instead of runtime state.
---

# Unity HotUpdate Boundary

Use this skill for any task that touches:
- `App.Shared`
- `App.AOT`
- `App.HotUpdate`
- YooAssets loading
- `MinesweeperTerrainData`
- gameplay architecture
- subagent task assignment for this project

## Goal

Keep the project structurally stable while gameplay is added.  
Prioritize clean boundaries over local convenience.

## Core Rules

- `App.AOT` is host infrastructure only.
- `App.AOT` may initialize services, platform bridges, persistence, YooAssets, HybridCLR, and shared infrastructure.
- `App.AOT` must not contain find-cat gameplay rules, board generation, task generation, reward formulas, or UI business flow.

- `App.Shared` is for stable cross-layer contracts only.
- Put only DTOs, interfaces, and cross-layer events in `App.Shared`.
- Do not put gameplay service implementations, Unity scene logic, or `ScriptableObject` logic in `App.Shared`.

- `App.HotUpdate` is the gameplay layer.
- Put gameplay services, board logic, task logic, progression logic, orchestration, and presenters here.
- Prefer pure C# domain logic that can be tested without Unity scene objects.

- `MonoBehaviour` is presentation glue, not gameplay authority.
- UI classes may forward input, bind state, and refresh visuals.
- UI classes must not own reward calculation, task generation, board generation, or persistence rules.

- `MinesweeperTerrainData` is a static board template.
- It may define rows, cols, valid cells, and block colors.
- It must not hold runtime mission progress, spawned cats, rewards, or save state.

- Runtime assets must load through YooAssets in formal gameplay flows.
- Do not introduce direct `Resources.Load` for new formal runtime features.
- Do not use editor-only APIs in runtime gameplay code.

## Required Workflow

When implementing a feature:
1. Identify which layer owns the change.
2. Freeze or confirm DTO/interface changes before parallel work.
3. Keep domain logic in pure C# services/models.
4. Keep Unity object code thin.
5. Verify resource access path and persistence ownership.
6. Review for boundary violations before finishing.

## Ownership Guide

Use these ownership defaults:
- `App.Shared`: only when a type must cross AOT/HotUpdate boundary.
- `App.AOT`: only when integrating platform, patching, boot, persistence, or shared infrastructure.
- `App.HotUpdate`: default home for feature work.

If unsure, prefer `App.HotUpdate`.

## Project-Specific Decisions

For this project:
- `MinesweeperTerrainData` is a map template input.
- Runtime board state should live in structures such as `BoardTemplate`, `LevelSnapshot`, `TaskInstanceData`, or equivalent runtime DTOs.
- Task generation depends on player-level config and current task slot rules, not on terrain assets.
- Terrain color is visual input only, not gameplay authority.
- Reward formulas belong to gameplay services, not UI.
- Ad unlock timers and offline settlement belong to time/persistence/meta services, not presenters.

## Subagent Rules

When splitting work across subagents:
- Every subagent should inherit this skill.
- Only one agent may lead changes in `App.Shared`.
- Only one agent may lead changes in HotUpdate entry/composition root.
- Only one UI-focused agent should change prefabs or scene bindings.
- Ask for DTO freezing before parallel implementation if multiple agents depend on shared data.

## Do Not

- Do not place gameplay rules in `App.AOT`.
- Do not place runtime mutable state in config assets.
- Do not put gameplay formulas in UI scripts.
- Do not bypass YooAssets for formal runtime content.
- Do not let multiple agents edit `App.Shared` casually.
- Do not expand prototype `Minesweeper` UI scripts directly into the final production architecture without refactoring ownership.

## Holmas 当前长期禁止项

这组禁止项是 Holmas 正式开发的长期规则源；执行时由 `unity-hotupdate-boundary` skill 镜像承接。

- 不要跳过边界冻结直接并行开发。
- 不要把运行时状态写回 `MinesweeperTerrainData`。
- 不要把奖励公式、地图生成、任务生成写进 UI。
- 不要让多个 agent 同时改 `App.Shared`。
- 不要再把任务领奖、地图完成或离线结算当成玩家经验来源。

## Validation

Before finishing:
- Check layer ownership.
- Check runtime/editor API separation.
- Check that Unity views stay thin.
- Check that mutable runtime state is not stored in template/config assets.
- Check that cross-layer types in `App.Shared` are minimal and stable.

Read these references when needed:
- `references/boundaries.md`
- `references/checklist.md`

Run this when doing a boundary-sensitive task:
- `tools/validation/check_boundary.sh`
```

## References

### `references/boundaries.md`

建议内容：
- 当前项目三层职责说明
- `MinesweeperTerrainData -> BoardTemplate -> LevelSnapshot` 的固定解释
- 允许写入和禁止写入示例
- 典型错误示例：
  - 在 UI 里生成任务
  - 在 Shared 放 ScriptableObject
  - 在 AOT 放找猫规则
  - 在配置资产里保存运行时进度

### `references/checklist.md`

建议内容：
- 新功能开发前检查：
  - 这次改动属于哪一层
  - 是否真的需要改 Shared
  - 是否需要 YooAssets key
  - 是否会引入运行时可变状态
- 提交前检查：
  - 逻辑是否在纯 C# 中
  - UI 是否只做展示
  - 是否绕过正式资源入口
  - 是否污染模板资产
  - 是否破坏 subagent 写入边界

## Script

### `tools/validation/check_boundary.sh`

当前仓库提供 3 个入口：

- `tools/validation/check_boundary.py`
- `tools/validation/check_boundary.sh`
- `tools/validation/check_boundary.bat`

默认命令：

- `python3 tools/validation/check_boundary.py`
- `tools/validation/check_boundary.sh`

第一版脚本保持轻量规则检查，不做自动修复。当前默认检查：

- `App.AOT / App.Shared / App.HotUpdate` 是否出现越层实现引用
- `App.HotUpdate / App.Shared` 是否混入 `UnityEditor` / `AssetDatabase` / `EditorUtility`
- 正式运行时代码是否出现 `Resources.Load` / `Resources.LoadAsync`
- 非 Editor / 非 Tests 代码是否对 `MinesweeperTerrainData` 做可写调用
- `App.Shared` 是否扩出可疑的 `Service / Manager / Runtime / Bootstrap / Coordinator / Loader` 实现类

输出风格：
- 列出命中项
- 区分 `failure` / `warning` / `info`
- 支持 `--json`、`--scope`、`--fail-on-warning`
- 不直接自动修复

## `openai.yaml`

先只需要最小元数据：
- `display_name`: `Unity HotUpdate Boundary`
- `short_description`: 约束 Unity + HybridCLR + YooAssets 项目的分层、资源、UI 和运行时状态边界
- `default_prompt`: 审查并遵循本项目的 AOT/Shared/HotUpdate 边界规则，优先保持 gameplay 逻辑纯 C#、UI 轻量、资源走 YooAssets

## Test Plan

这份 skill 的第一轮验证用 4 类任务测试：

- 改 `App.Shared` DTO 时，skill 能提醒“只放跨层稳定类型”
- 做地图生成时，skill 能提醒“运行时状态不要写回 `MinesweeperTerrainData`”
- 做 UI 接线时，skill 能提醒“Presenter 不能承载奖励公式和任务生成”
- 做 subagent 分工时，skill 能提醒“Shared、入口、Prefab 必须独占”

如果这 4 类任务都能稳定约束住，第一版就算合格。

## Assumptions

- 这版先做“能用”的第一版，不追求一次覆盖所有长期规则。
- 第一版只做总边界 skill，不同时展开配置 skill 和 UI skill。
- 允许后续把更细的项目规则下沉到 `references/`，保持 `SKILL.md` 短而硬。
- 真正落地时，建议先创建这个 skill，再让所有 subagent 默认继承它。

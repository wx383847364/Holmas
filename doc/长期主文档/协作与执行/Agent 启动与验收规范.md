...
- 同一条审查链里，默认优先复用原 Agent 6 持续负责复审；只有原实例超时、不可用或上下文明显失真时，才允许同职责 reviewer 接手同一 `review_chain_id`
...
- `review_deferred` 只表示“同步等待已超时或 reviewer 暂不可用；当前允许继续已知修复与验证，但不能宣告阶段通过”
...
- 主线程必须同步说明：
  ...
  - 是否已经送回同一审查链复审
- 只有当审查已发起、同步等待超时，或当前 reviewer 暂不可用但审查链仍有效时，才允许把状态写成“待回补复审”
...
- 主线程只有在“所有相关阶段里程碑都已通过 Agent 6”后，才允许做最终汇总、阶段结论和下一阶段建议
...
- 这类情况下要把 execution 状态记成 `review_deferred`，并把对外文案明确写成“待回补复审”，而不是误写成“已通过”或“等待继续开发”

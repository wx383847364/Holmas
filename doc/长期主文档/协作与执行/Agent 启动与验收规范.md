...
主线程等待真实 subagent 结果时，必须区分“快速探测”和“需要结果才能判断”的两种场景。

- 只做“快速探测是否已完成”时，等待时间不得低于 30 秒；不再使用 10 秒短等待。
- 排查、验证、代码阅读、审查类 subagent，默认等待 1-3 分钟；复杂验证链或长日志排查可等待更久。
- 如果主线程需要 subagent 结果才能继续决策，应等待到结果返回或明确超时；不能用一次短等待 timeout 代表 subagent 缺席。
- 如果主线程选择先不等待，应明确说明“先继续主线程工作，等 subagent 通知回来后再合并结论”。
- subagent 通知晚于主线程回复时，后续回复必须合并最新通知结果，不能沿用旧的 timeout 判断。

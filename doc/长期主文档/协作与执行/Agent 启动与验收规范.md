...
- 是否明确给出 `通过 / 通过，但有非阻塞建议 / 不通过，退回修复` 之一
...
5. 通过基础验收后，默认交给 Agent 6 做挑刺与问题审查
6. Agent 6 未通过时，退回原实现方修复；修完后默认交回同一个 Agent 6 复审
7. 只有 Agent 6 通过后，才决定是否启动下一个 agent、是否追加 Agent 5 的专项验证，或进入下一个阶段
8. 本轮结束后，执行文档维护流程

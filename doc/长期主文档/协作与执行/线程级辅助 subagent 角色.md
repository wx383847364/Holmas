...
- 执行型 `Agent 1 ~ 6` 的职责、写入边界和验收规则统一看 [Agent 启动与验收规范](Agent%20启动与验收规范.md)
- helper role 负责：
  - 读取长期主文档、迭代记录和必要代码入口
  - 输出主线判断、阻塞判断、下一步建议
  - 审计协作规则、收尾规则和流程漏洞
...
新会话只要已经按 [Codex新会话必读](Codex新会话必读.md) 进入默认入口，主线程就必须自己判断是否需要 helper，不需要用户额外补口令。
...
执行型记录的字段和状态语义仍以 [Agent 启动与验收规范](Agent%20启动与验收规范.md) 为准；本页只补 helper 专属要求。
...
- 如需显式覆盖默认行为，再看 [skill 与 subagent 任务模板](skill%20与%20subagent%20任务模板.md) 中的 helper 模板
- helper 只是线程内辅助判断机制，不改变官方执行型 Agent 分工

## 与其他长期文档的关系

- 新会话默认入口继续看 [Codex新会话必读](Codex新会话必读.md)
- 执行型真实 subagent 的启动、调度、职责、验收和 Agent 6 闭环继续看 [Agent 启动与验收规范](Agent%20启动与验收规范.md)
- helper 的覆盖模板继续看 [skill 与 subagent 任务模板](skill%20与%20subagent%20任务模板.md)
- 文档收尾继续看 [任务完成后自动维护文档](任务完成后自动维护文档.md)
- Git 提交建议格式、确认词和 `commit / push` 口径继续看 [Git 提交建议与确认规则](Git%20提交建议与确认规则.md)

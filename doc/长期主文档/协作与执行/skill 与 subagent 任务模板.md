...
- 某条实现线达到阶段里程碑后，默认交给 Agent 6 做挑刺审查
- 如果 Agent 6 不通过，必须先按职责归属路由回实现线；修完后默认优先交回同一审查链复审
...
先冻结 DTO，再并行开发；每个阶段里程碑默认先交给 Agent 6 挑刺审查，通过后再宣告该里程碑完成或切入下一阶段。
如果 Agent 6 不通过，先退回原实现方修复；如果当前没有原实现真实 subagent，就自动补起同职责真实 subagent 接手修复。
修完后默认优先交回同一审查链复审；如果原审查实例超时或不可用，再由同职责 reviewer 接手。
```
...
- 每条功能线达到阶段里程碑后，先交给 Agent 6 做挑刺审查；不通过就退回原实现线修复，修完后默认优先交回同一审查链复审

...
说明：
- 这份文档是长期拆分参考，不直接作为迭代记录生成源
- 迭代记录里的 `分工状态` 默认以 6-Agent 实操版为准
- 如无特别说明，不直接从这里的 9-agent 文本生成迭代状态

## Summary

按你刚刚确认的“完整长期版”范围来拆，我建议用 **9 个执行型 subagent + 1 个主控 agent**。  
这个数量是比较稳的平衡点：

- 少于 9 个，会把“测试验证”和“独立挑刺审查”混成一个角色，后期很容易既写测试又自己放过问题。
- 多于 9 个，真正会争抢的写入面会迅速增多，尤其是 `App.Shared`、`HotUpdate` 组合层、UI 预制体和存档模型。
...
### 5. Detective Agency / Progression agent
...
### 9. Critic / Defect Review agent

职责：
- 独立挑刺，找明显 bug、回归、越界、误解需求、缺关键验证和交接缺失
- 对每个阶段里程碑交付给出 `通过 / 通过，但有非阻塞建议 / 未通过，退回修复`
- 不直接接管实现，只负责裁定和退回
...
5. 每个阶段里程碑交付都必须先经过 Critic / Defect Review agent 审查；中间修复和局部验证默认继续当前实现/验证链，不同步卡住主线。
...
9 `Critic / Defect Review` 在每个阶段里程碑交付后都要介入。  
它不做实现，只做挑刺、裁定和退回修复；如果同步等待超时，主线状态转为 `待回补复审`，继续已知修复与验证。
...
7. 接入 7 号 UI 全流程
8. 8 号做完整回归和压力验证
9. 9 号对阶段里程碑交付做阻塞性审查，未过审不进入下一阶段
...
- 原 reviewer 超时后，新 reviewer 接手同一 `review_chain_id` 时，脚本口径和线程级注册表口径保持一致，且不会把 `待回补复审` 误写成 `已通过`
- Critic / Defect Review 能对每个阶段里程碑交付给出明确阻塞结论

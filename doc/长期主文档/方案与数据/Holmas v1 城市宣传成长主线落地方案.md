...
- 当前状态：进行中
- 进度说明：已修复当前 5 城市分页满级后地图未刷新到下一页的问题，并保持底部 promotion 跟随最新推进城市。
- 最近更新：2026-05-10，已修复当前 5 城市分页满级后地图未刷新到下一页的问题，并保持底部 promotion 跟随最新推进城市。
...
- 底部 promotion 按钮必须使用当前最新推进城市的 `stageImage`，不得因为点击地图历史/未来城市而刷新到底部旧状态
...
当总阶段数大于 `5` 时，界面按当前最新推进阶段所在分页生成连续的 `5` 个可见 stage：

- 地图区域仍然始终保留 `Stage1..Stage5` 这 5 个 stage slot
- 每页按表顺序显示连续 5 个城市，例如 `1..5`、`6..10`
- 点击当前分页内的已配置 stage 只切换地图查看/选中城市，不刷新底部 promotion 列表
- 当前 5 城市分页全部完成并解锁下一页后，地图刷新到最新推进城市所在分页；底部 promotion 列表保持最新推进城市状态

### 底部 promotion 图标与星级

- `Build_btn` 作为底部 promotion slot 的容器，不再表示单个固定城市建设按钮
- 底部图标数量读取当前最新推进城市 stage 的 `promotionLevelCaps` 字段
- `promotionLevelCaps` 有几个元素，底部就显示几个 promotion 图标；超出静态 slot 数量时可在 `Build_btn` 直接子节点下池化补充
- `promotionLevelCaps[i]` 表示第 `i` 个底部 promotion 图标的星星总数
- 星星点亮数量等于对应 promotion 当前等级，范围固定为 `0..promotionLevelCaps[i]`
- 不再使用固定 `5` 星算法，也不再把底部图标理解为地图 5 个城市
- 当前城市是否满级按该城市所有 `promotionLevelCaps` 总和判断
  - 例如某城市总 cap 为 `15`，显示 `15/15` 后才算该城市满级
  - 满级后推进到下一 `agencyStageId`，下一城市 stage 解锁并隐藏锁定态，底部 promotion 同步为新的最新推进城市
- 底部 promotion 始终代表当前最新推进城市，可升级状态也只按当前最新推进城市判断
- 点击历史/未来地图城市时，底部 promotion 不切换到被点击城市

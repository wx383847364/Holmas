...
LOG_PREFIX="ui_prefab_generator_tests_$(date +%Y%m%d_%H%M%S)"
LOG_DIR="/tmp"
TEST_METHOD="UiPrefabGenerator.Editor.Validation.UiPrefabGeneratorValidationMenu.RunUiPrefabGeneratorEditModeTests"
TEST_SUCCESS_PATTERN="UiPrefabGenerator EditMode tests finished\\."
...
说明：
  ...
  - 只跑 UiPrefabGenerator.Tests 的 EditMode 测试
  - 通过后默认清理临时工程
EOF
}
...
if [[ -z "${TEMP_PROJECT_DIR}" ]]; then
    TEMP_PROJECT_DIR="$(mktemp -d "/private/tmp/ui_prefab_generator_validation_XXXXXX")"
    TEMP_CREATED_BY_SCRIPT=1
else
    mkdir -p "${TEMP_PROJECT_DIR}"
fi

TEST_LOG="${LOG_DIR}/${LOG_PREFIX}_editmode.log"
...
log "开始执行 UiPrefabGenerator EditMode 验证..."
run_batchmode "${TEST_METHOD}" "${TEST_LOG}"
assert_log_contains "${TEST_SUCCESS_PATTERN}" "${TEST_LOG}"
assert_log_contains "Exiting batchmode successfully now!" "${TEST_LOG}"
log "UiPrefabGenerator EditMode 验证通过，日志：${TEST_LOG}"
log "UiPrefabGenerator 验证全部通过。"

...
{
    public static string CreateDefaultPackageRoot(string packageName)
    {
        return $"{WX.env.USER_DATA_PATH}/__GAME_FILE_CACHE/yoo/{packageName}";
    }

    public static FileSystemParameters CreateFileSystemParameters(string packageRoot, IRemoteServices remoteServices)
    {
        string fileSystemClass = $"{typeof(WechatFileSystem).FullName},{typeof(WechatFileSystem).Assembly.GetName().Name}";
...
internal class WechatFileSystem : IFileSystem
{
    ...
    public virtual void OnCreate(string packageName, string packageRoot)
    {
        ...
        // Holmas 微信小游戏必须走显式 HTTPS CDN，禁止回退到 Application.streamingAssetsPath。
        if (RemoteServices == null)
        {
            throw new Exception("WechatFileSystem requires explicit IRemoteServices. Configure Assets/Settings/HolmasWeixinMiniGameCdnSettings.json with an HTTPS CDN root.");
        }
